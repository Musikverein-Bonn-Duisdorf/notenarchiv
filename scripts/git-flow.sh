#!/usr/bin/env bash
# Notenarchiv: pull / push / PR / merge helpers.
#
...
#   ./scripts/git-flow.sh pr-dev --title "ARCHIV-n: …" --body "## Summary\n…"
...
REMOTE="${NOTENARCHIV_REMOTE:-origin}"
BRANCH_DEV="${NOTENARCHIV_BRANCH_DEV:-dev}"
BRANCH_MASTER="${NOTENARCHIV_BRANCH_MASTER:-master}"
...
cmd_merge_pr() {
  ...
  # Match Meldeliste style: merge commit into BRANCH_DEV.
...
Env overrides: NOTENARCHIV_REMOTE, NOTENARCHIV_BRANCH_DEV, NOTENARCHIV_BRANCH_MASTER
EOF
}
