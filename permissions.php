...
$sql = sprintf(
    'SELECT `Index` FROM `%sUser` WHERE (`Deleted` IS NULL OR `Deleted` != 1) ORDER BY `Nachname`, `Vorname`;',
    identityPrefix()
);
...
while($row = mysqli_fetch_array($dbr)) {
    ...
    $hasMeldeAccess = IdentityPermissions::loadForUser($uid)->getPermission('perm_accessNotenarchiv');
    $local = ArchivPermissions::loadForUser($uid);
    if(!$hasMeldeAccess && !$local->hasAnyPermission() && (int)$local->Index < 1) {
        continue;
    }
    $user = new User;
    $user->load_by_id($uid);
    ...
    $rows[] = array(
        ...
        'perm' => $local,
        'name' => $user->getName(),
        'hasAny' => $local->hasAnyPermission(),
        'meldeAccess' => $hasMeldeAccess,
    );
}
...
<?php if(empty($entry['meldeAccess'])) { ?>
              <span class="profile-label" title="Kein Melde-Recht Notenarchiv">(ohne Melde-Zugang)</span>
<?php } ?>
            </td>
<?php foreach($permKeys as $key) {
...
?>
            <td class="perm-cell perm-group perm-group--<?php echo htmlspecialchars($gid, ENT_QUOTES, 'UTF-8'); ?> <?php echo $on ? 'perm-on' : 'perm-off'; ?>" title="<?php echo $title; ?>">
              <input type="checkbox"
                     ...
                     <?php echo $on ? 'checked' : ''; ?>
                     <?php echo $locked ? 'disabled'.$disabledTitle : ''; ?>>
            </td>
<?php } ?>
          </tr>
<?php } ?>
