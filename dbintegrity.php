<?php
/**
 * HTML report for SchemaManager results (used by updater.php / install.php).
 */
...
function DBRenderReport($report, $notableOnly = true) {
    if($notableOnly && is_array($report)) {
        ...
        foreach($report as $entry) {
            ...
            if(class_exists('SchemaManager') && SchemaManager::isNotableStatus($status)) {
                $filtered[] = $entry;
            }
            elseif(!class_exists('SchemaManager') && $status !== 'ok') {
                $filtered[] = $entry;
            }
        }
        $report = $filtered;
