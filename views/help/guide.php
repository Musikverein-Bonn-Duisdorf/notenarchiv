...
'.($showAdmin ? '<p>Admins legen neue Stücke über <b>Admin → Stück</b> bzw. Plus auf der Stückliste an (<code>composition.php</code>). Komponisten und Arrangeure werden als Chips zugeordnet (mehrere möglich); fehlende Personen und Verlage per Plus daneben. Sammlungszuordnung per Chips mit Speichern. Sammlungen, Komponisten und Verlage haben jeweils eigene Anlege-Seiten (Avatar/Logo, weitere Felder).</p>' : '').'
'
);
...
$sections[] = array(
    ...
    'title' => 'Sammlungen',
    'body' => '
<p>Unter <b>Sammlungen</b> verwaltest du Zusammenstellungen (z.&nbsp;B. Ordner im Schrank oder thematische Listen). Stücke können einer oder mehreren Sammlungen zugeordnet sein. Die Liste startet zugeklappt (Stückzahl am Titel); Aufklappen zeigt den Inhalt. Die Suche filtert Sammlungen nach Name; bei Namens-Treffer bleibt der Inhalt sichtbar. Suche nach Stücktiteln findet Sammlungen mit Treffer und blendet dort nicht passende Stücke aus. Standard-Sortierung: neueste zuerst (ID). Archivierte Sammlungen sind standardmäßig ausgeblendet; der Filter <b>Archivierte</b> zeigt sie wieder'.($showAdmin ? '. <b>Bearbeiten</b> öffnet die Anlege-Seite (Flags <b>Archiviert</b> und <b>Nummeriert</b>). Bei nummerierten Sammlungen erscheint die Sammlungsnummer als Badge vor jedem Stück; die Reihenfolge folgt der Nr. im Chip-Editor. Ohne Nummeriert bestimmst du die Reihenfolge per Auf/Ab. Am Stück (Edit) erscheint die Nr. nur bei nummerierten Sammlungen. Dort und im Sammlungen-Dialog am Stück pflegst du die Zuordnung per Chips. Anlegen über Plus bzw. <b>Admin → Sammlung</b>' : '').'.</p>
'
);
...
$sections[] = array(
    ...
    'body' => '
<p><b>Komponisten</b> und <b>Verlage</b> sind Stammdaten für den Katalog (Sortierung und Nachladen wie bei den Stücken). Anlegen geht über die jeweiligen Anlege-Seiten (<b>Admin → Komponist</b> / <b>Verlag</b> oder Plus in der Liste) sowie schnell beim Stück per Plus. Ein Stück kann mehrere Komponisten und Arrangeure haben (Chip-Felder; Reihenfolge = Anzeige). Verlage können Website und Logo haben. Stücke können eine <b>Produktseite</b> (konkrete Verlags-URL) und optional eine <b>Aufnahme</b> (YouTube, Aufnahme oder Nextcloud-mp3) haben; ist sie gesetzt, erscheint rechts in der Stückzeile ein Volume-Symbol, das die Aufnahme in einem neuen Tab öffnet (Liste, Detail, Stückseite). Der Verlagsname verlinkt zur Produktseite bzw. auf die Verlags-Website. Ein Klick in der Liste öffnet ein Detail-Modal; <b>Bearbeiten</b> führt auf die Anlege-Seite (dort auch Löschen und Foto).</p>
'
);
...
<li><b>Backup</b> – zwei ZIPs: Datenbank (<code>manifest.json</code> + <code>database.sql</code> nur für <code>archiv_*</code>, nicht Melde-Identity) und Dateien unter <code>data/</code> (PDFs, Cover, Avatare). Browser: Backup / Dateien laden; Restore mit <code>RESTORE</code> bzw. <code>FILES</code> (Dateien ersetzen <code>data/</code> als Snapshot). CLI: <code>php cron.php CRONID backup</code> bzw. <code>backup-files</code>; Restore <code>php scripts/restoreBackup.php backup.zip --yes</code> (erkennt DB- vs. Datei-ZIP). Remote nur mit eigenem <code>$backupToken</code> (≥32 Zeichen) über <code>cron.php?id=…&amp;cmd=backup</code> bzw. <code>cmd=backup-files</code>. Browser-Restore ist durch PHP <code>upload_max_filesize</code> begrenzt; große Datei-ZIPs per CLI. Erfolgreiche Downloads erscheinen im Log als Info, Fehler als Error</li>
